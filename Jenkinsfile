    ...
    stage('Docker Build & Push') {
        steps {
            withCredentials([string(credentialsId: 'DOCKER_HUB_PASSWORD_ACHRAF', variable: 'DOCKER_HUB_PASSWORD')]) {
                ...
                sh 'docker build -t hrefnhaila/nodejsachraf:$LOCAL_TAG .'
                sh 'docker push hrefnhaila/nodejsachraf:$LOCAL_TAG'
            }
        }
    }
    ...
    stage(' Deployment docker ') {
        steps {
                ...
                script {
                    ...
                    sh 'docker run -d  -p 3333:3000 --name hrefnhaila$LOCAL_TAG hrefnhaila/nodejsachraf:$LOCAL_TAG'
                }
